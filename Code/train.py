...
def prepare_MNIST(device, load=False, model_name=''):
    ...
    trainset = torchvision.datasets.MNIST(root='./data/MNIST', transform=transform, train=True, download=False)
    testset = torchvision.datasets.MNIST(root='./data/MNIST', transform=transform, train=False, download=False)
    ...
    optimizer = optim.SGD(net.parameters(), lr=1.00e-02, momentum=0.9, weight_decay=1.00e-04, nesterov=True)
    #optimizer = optim.Adam(net.parameters(), lr=1.00e-02, weight_decay=1.00e-04)
...
def prepare_CIFAR_10(device, load=False, model_name='', augment=True):
    ...
    if(load==False):  
        net=RCNN(in_channels=3,feature_num=96,feature_map_width=feature_map_width,iter_time=3,device=device)
        #for rcnn
        #net = RCNN(3, 10, 96)
    else:
        net=torch.load(model_name)
    ...
    criterion = torch.nn.CrossEntropyLoss()
    optimizer = optim.SGD(net.parameters(), lr=1.00e-01, momentum=0.9, weight_decay=1.00e-04, nesterov=True)
    #optimizer = optim.Adam(net.parameters(), lr=1.00e-02, weight_decay=1.00e-04)
    net=net.to(device)
    return batch_size, batch_size_test, net, trainset, testset, trainloader, testloader, criterion, optimizer
