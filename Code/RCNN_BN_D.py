...
class RCNN(torch.nn.Module):
    def __init__(self, in_channels, feature_num, feature_map_width, iter_time, device):
        ...
        self.mxp5 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
class RCL(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, iter_time, feature_map_width, device, stride=1, padding=1):
        ...
        self.bn=[]
        for i in range(0, self.iter_time+1):
            self.bn.append(torch.nn.BatchNorm2d(out_channels))

        for i in range(0, self.iter_time+1):
            self.bn[i].to(device)
            self.add_module('bn[%d]'%(i),self.bn[i])
